...
always_ff @(posedge clk or negedge rst_n) begin
	if(~rst_n)			current_state <= FETCH;	else
						current_state <= (current_state == FETCH) ? EXECUTE : FETCH;
end
...
assign write_en 	= 	current_state & (is_add | is_sub | is_and | is_or | is_ldi |
						is_shl | is_shr | is_load);
assign jump_en = is_jmp | (is_jmpz & zero_flag_reg);
assign alu_opcode 	= 	opcode[2:0];
assign use_imm 		= 	is_ldi;
assign mem_write_en = 	current_state & is_store;
assign mem_to_reg	= 	is_load;
assign pc_en		=	current_state;
